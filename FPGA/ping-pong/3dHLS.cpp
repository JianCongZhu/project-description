...
void hotspot_HW(ap_uint<LARGE_BUS> result[GRID_COLS * GRID_ROWS * LAYERS], ap_uint<LARGE_BUS> temp[GRID_COLS * GRID_ROWS * LAYERS], ap_uint<LARGE_BUS> power[GRID_ROWS * GRID_COLS * LAYERS], float Cap, float Rx, float Ry, float Rz, float dt, float amb_temp)
{
  ...
  for (i = 0; i < ITERATIONS / 2; i++)
  {
    ...
    for (j = 0; j < LAYERS; j++)
    {
      selectLayer(temp, center_buf, top_buf, bottom_buf, j);
    ...
    for (j = 0; j < LAYERS; j++)
    {
      selectLayer(result, center_buf, top_buf, bottom_buf, j);
      buffer_load(power_buf, power + GRID_ROWS * GRID_COLS * j / 16);
      compute(result_buf, center_buf, top_buf, bottom_buf, power_buf, cc, cn, cs, ce, cw, ct, cb, Cap, dt, amb_temp, i);
...
void selectLayer(ap_uint<LARGE_BUS> temp[GRID_COLS * GRID_ROWS * LAYERS], float center_buf[GRID_ROWS * GRID_COLS], float top_buf[GRID_ROWS * GRID_COLS], float bottom_buf[GRID_ROWS * GRID_COLS], int layer)
{

  if (layer == 0) // bottom case
  {
    buffer_load(center_buf, temp + (GRID_ROWS * GRID_COLS * layer) / 16);    // load for center layer
    buffer_load(top_buf, temp + (GRID_ROWS * GRID_COLS * (layer + 1) / 16)); // load for top layer
    buffer_load(bottom_buf, (temp + GRID_ROWS * GRID_COLS * layer / 16));    // load for bottom layer
  }
  else if (layer == LAYERS - 1) // top case
  {
    buffer_load(center_buf, temp + (GRID_ROWS * GRID_COLS * layer) / 16);       // load for center layer
    buffer_load(top_buf, temp + (GRID_ROWS * GRID_COLS * layer) / 16);          // load for top layer
    buffer_load(bottom_buf, temp + (GRID_ROWS * GRID_COLS * (layer - 1) / 16)); // load for bottom layer
  }
  else
  {
    buffer_load(center_buf, temp + (GRID_ROWS * GRID_COLS * layer) / 16);     // load for center layer
