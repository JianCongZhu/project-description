...
void buffer_load(float *dest, float *source, int layer)
{
  float[GRID_ROWS*GRID_COLS] temp;
  for(int i = 0; i < GRID_ROWS*GRID_COLS; i++){
    temp[i]=source[i*8+layer];
  }
...
void buffer_store(float *dest, float *source, int layer)
{
  float[GRID_ROWS*GRID_COLS] temp;
  for(int i = 0; i < GRID_ROWS*GRID_COLS; i++){
    temp[i*8+layer]=source[i];
  }
  memcpy(dest,source,sizeof(float) * GRID_ROWS * GRID_COLS);
  ...
  for (i = 0; i < (ITERATIONS / 2); i++)
  {
    for (j = 0; j < LAYERS; j++)
    {
      ...
      if (j == 0) // bottom case
      {
        ...
        buffer_load(center_buf, temp , j);    // load for center layer
        buffer_load(top_buf, temp , (j + 1)); // load for top layer
        buffer_load(bottom_buf, temp , (j));  // load for bottom layer
      }
      else if (j == LAYERS - 1) // top case
      {
        buffer_load(center_buf, temp, j);       // load for center layer
        buffer_load(top_buf, temp , (j));        // load for top layer
        buffer_load(bottom_buf, temp , (j - 1)); // load for bottom layer
      }
      else
      {
        buffer_load(center_buf, temp , j);       // load for center layer
      ...
      buffer_load(power_buf, power , j);
      // printf("here after load 2\n");
      compute(result_buf, center_buf, top_buf, bottom_buf, power_buf, cc, cn, cs, ce, cw, ct, cb, stepDivCap, dt, amb_temp,i);
      // printf("here after compute\n");
      buffer_store(result, result_buf,j);
    ...
    for (j = 0; j < LAYERS; j++)
    {
      ...
      if (j == 0) // bottom case
      {
        buffer_load(center_buf, result , j);    // load for center layer
        buffer_load(top_buf, result ,(j + 1)); // load for top layer
        buffer_load(bottom_buf, result, (j));  // load for bottom layer
      }
      else if (j == LAYERS - 1) // top case
      {
        buffer_load(center_buf, result, j);       // load for center layer
        buffer_load(top_buf, result , (j));        // load for top layer
        buffer_load(bottom_buf, result , (j - 1)); // load for bottom layer
      }
      else
      {
        buffer_load(center_buf, result , j);       // load for center layer
      ...
      compute(result_buf, center_buf, top_buf, bottom_buf, power_buf, cc, cn, cs, ce, cw, ct, cb, stepDivCap, dt, amb_temp, i);
      // printf("here after compute\n");
      buffer_store(temp, result_buf, j);
      // printf("here after store\n");
    }
